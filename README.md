...
| 8 | `serve` 深化 | 地址清单分组（可改分组、按组过滤）、JSON 导入导出；**全节点对比**（勾选地址 × 全部节点批量检测）；**多订阅网页管理**（添加/删除/⟳重新拉取多个订阅，持久化 `~/.netscope/subs.json`，跨订阅去重，节点自动进入通道下拉）；页面含 logo 与整体视觉优化；全部由按钮手动触发（评分趋势图与历史报告展示后续按使用需求移除） | ✅ |
...
| 8 | 检测语义修正 | 可达性三态：✅ 可访问(2xx/3xx) / 🟡 可达·被拒(4xx/5xx，如 Cloudflare 对机房 IP 的 403，网络层是通的) / ❌ 不可达(网络错误)；检测请求改用浏览器 UA 与标准 Accept 头，降低被防护误拦；常见网络错误给出可操作提示（被重置→建议换通道、超时等） | ✅ |
...
| 8 | 游戏平台连接 | serve 网页「🎮 游戏平台连接」双模块（等宽双列，各自独立选通道测试）：**Steam** 商店/社区连通性 + 货币区（store 页 priceCurrency，参考 RRC）；**PlayStation** 商店/账户/官网连通性 + 店面区域（store.playstation.com 重定向国家码，Sony 按出口 IP 分发，语言固定 en）；两平台共用一个「🌐 全节点检测」按钮（一次任务 × 两平台，行序检测中锁定、完成后按全通+延迟排序） | ✅ |
